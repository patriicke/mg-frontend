...
export default class NumberFormat {
  ...
  parts(num: InputType, separator?: string) {
    const { precision, minimumFractionDigits, decimal } = this.options
    let parts: InputType[] = num.toString().split(separator || decimal)
  ...
  addSeparator() {
    ...
    const parts: InputType[] = this.numbers().split(decimal)
   ...
   * @return {String}
   */
  unformat(input: InputType): string {
    this.input = input
