...
const CryptoCarousel: React.FC<IProps> = ({ cryptos, title }) => {
  ...
  const addCryptoToSlot = (nft: any, winRate: number) => {
    setFairness({
      clientSeed: "",
      serverSeed: "",
      currentServerSeed: "[]",
  ...
  const redirectToCryptoListing = () => {
    navigate(`/cryptoslot/${cryptos[0].contractAddress}`);
  };
    ...
    <div className="mt-5">
      <div className="flex justify-between w-full">
        ...
        <div className="flex items-center justify-center space-x-3">
          <p
            ...
            onClick={redirectToCryptoListing}
          >
            ALL {cryptos?.length}
          </p>
          <button
          ...
          <button
            onClick={nextSlide}
            disabled={
              currentSlide === cryptos.length - (isMobile ? 3 : isTablet ? 4 : 5)
            }
            className={cx(
              "px-2 py-1 border border-[#484848] bg-[#484848] rounded-md text-black",
              {
                "cursor-not-allowed":
                  currentSlide ===
                  cryptos.length - (isMobile ? 3 : isTablet ? 4 : 5),
      ...
      <div className="w-full overflow-hidden mt-5">
        <div
          ...
          style={{
            gridTemplateColumns: `repeat(${cryptos.length}, ${
