...
const StatusItem: React.FC<StatusItemProps> = ({
  ...
  const minButtonHandler = () => {
    ...
    if (title === "Price") {
      const newPrice =
        (parseFloat((0.01).toString()) / 100) *
        parseFloat(cryptoPrice.toString());
  ...
  const winRateChangeHandler = (value: string) => {
    ...
    setValue(value);
    cryptoDispatch({
      type: CRYPTOActionType.SET_WIN_RATE,
      payload: value,
    });
    ...
    if (roundOff(value, 2) <= 98 && roundOff(value, 2) >= 0.01) {
      const newPrice = roundOff((parseFloat(value) / 100) * cryptoPrice, 2);
  ...
  const priceChangeHandler = (value: string) => {
    setValue(value);
    cryptoDispatch({
      type: CRYPTOActionType.SET_BET_PRICE,
      payload: value,
    });
    if (!value) {
      cryptoDispatch({
    ...
    if (roundOff(value, 2) >= 0 && roundOff(value, 2) <= cryptoPrice) {
      const newWinRate = roundOff((parseFloat(value) / cryptoPrice) * 100, 2);
