...
import { CryptoBetHistoryData } from "interface/crypto/crypto.interface";
...
import { getAllCryptoBets, getPersonalCryptoBets } from "services/crypto.service";
...
const MyBet: React.FC<IProps> = ({ setCurrentTab }) => {
  const [historyData, setHistoryData] =
    useState<CryptoBetHistoryData>(initialHistoryData);
  ...
  const getHistory = async (pageNumber: number) => {
    ...
    if (tab === ALL_BETS_VALUE) {
      [response, error] = await getAllCryptoBets(pageNumber);
    }
    if (tab === MY_BETS_VALUE) {
      [response, error] = await getPersonalCryptoBets(pageNumber);
    }
    if (response) {
      ...
      <div className="w-full">
        <div className="w-full my-[14px] font-normal">
          <div className="w-full flex flex-row items-center text-[12px] leading-[15px] md:text-[18px] md:leading-[22px] text-[#F0BE43] text-left border-b border-[#484848]">
            <th
              className={cx("w-[30%] py-4 px-5 md:pl-[34px] font-normal", {
            ...
            <th
              className={cx("w-[25%] py-4 font-normal", {
          ...
          <div className="grid grid-cols-1 items-center text-left text-[10px] leading-[15px] md:text-[20px] md:leading-[25px] text-white divide-y divide-[#484848]">
            {historyData?.results.map((bet, index) => (
              ...
              >
                ...
                >
                  <div className="flex gap-[11px] sm:gap-[20px] lg:gap-[29px] items-center">
                    
                    <img src={bet.nftImage} className="w-[30px] h-[30px] lg:w-[84px] lg:h-[84px] rounded rounded-[6px]" alt="Game Image" />
                    <span className="font-[500] lg:font-[300] text-[14px] md:text-[20px]">
                        {bet.symbol}
                    </span>
                  </div>
                </div>
                <div
                  className={cx("w-[25%] flex flex-row items-center break-all", {
                    "w-[35%]": tab !== MY_BETS_VALUE,
                  })}
                >
                    <span className="font-[300] text-[14px] md:text-[20px] text-[#737373] lg:text-[#FFFFFF] capitalize ">
                ...
                >
                    <span className="text-[14px] md:text-[20px] text-[#B0D412] font-[500] lg:font-[300]">
